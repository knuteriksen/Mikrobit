...
void twi_init(){
    ...
    GPIO->PIN_CNF[0] = (0 << 0) | (2 << 3) | (6 << 8); // input, pullup, s0d1
	GPIO->PIN_CNF[30] = (0 << 0) | (2 << 3) | (6 << 8);
...
void twi_multi_read(
    ...
    for (int j = 0; j < (registers_to_read -1); j++ ) {
		while(!TWI0->RXDREADY); //vent til ferdig med å lese
		TWI0->RXDREADY = 0; 
		data_buffer[j] = TWI0->RXD; //send i databuffer
    }
